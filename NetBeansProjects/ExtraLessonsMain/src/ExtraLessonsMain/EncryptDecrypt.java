...
public class EncryptDecrypt {//creates a class called EncryptDecrypt to handle encryption and decryption of files and text
      ...
      public static String encrypt(String e) {// creates a methood that returns a incrypted String in: string to be encrypted
        ...
        for (int loop = 0; loop < e.length(); loop++) {//loops though the parsed in string one char at a time
            ...
            for (int r = 0; r <= 255; r++) {//loops through all the ascii values
                if (arr[r][indchar] == indkey) {//if the inetger at the posistion got by the loop equals the key
    ...
    public static String key(int l) {//the parameter takes in the length of the text in: lenghth of the text
    ...
    public static String decrypt(String e) {//decrypts the passed in text in: text to be decrypted
        int arr[][] = new int[256][256];//creates an interger 2D array [row][column] 
        for (int loop = 0; loop <= 255; loop++) {//row loop
            for (int k = 0; k <= 255; k++) {//column loop
                if (k + loop < 256) {
